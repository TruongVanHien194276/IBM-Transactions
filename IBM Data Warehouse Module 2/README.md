...
Project môn Data Engineering/Data Warehouse sử dụng bộ **IBM Transactions for Anti Money Laundering (AML) – HI-Medium**.
...
- Tải đúng ba file HI-Medium từ Kaggle và khóa bằng SHA-256
...
Local PostgreSQL hiện dùng `trust` cho localhost. Cấu hình này chỉ phù hợp máy học tập cá nhân, không dùng cho production
...
- Một dòng `HI-Medium_Trans.csv` là một giao dịch giữa tài khoản gửi và tài khoản nhận
...
- Tài liệu: `7. Technical docs/20_HuongDanChayTuan3_macOS_PostgreSQL_PowerBI.md`

### Mở dashboard Power BI trên macOS
...
Khi cần cập nhật dữ liệu snapshot, chạy:
...
Trong preview, menu `01`–`07` chỉ chuyển giữa các mockup thiết kế. Nếu file đang mở từ trước khi tạo lại, nhấn `Command + R` để tải phiên bản mới

Mở thư mục chứa ảnh PNG `3840 × 2160`:
